...
- After generation, resolve `scripts/remove_chroma_key.py` relative to the directory containing `SKILL.md`, run it with `--input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`, and validate the alpha result before shipping it.
